# model.py
#
# Primitive Dreamer 4 architectures (forward passes; backward via autograd).
# Vendored verbatim from the unofficial PyTorch implementation:
#   https://github.com/nicklashansen/dreamer4  (commit bdeddfe)
#   dreamer4/model.py
# Original work Copyright (c) 2026 Nicklas Hansen, MIT License.
# See LICENSE.dreamer4 in this directory for the full license text.
import math
...
class SpaceSelfAttentionModality(nn.Module):
    def __init__(self, d_model: int, n_heads: int, modality_ids: torch.Tensor, n_latents: int, mode: str, dropout: float):
        ...
        # An all-True mask (e.g. "wm_agent" full mixing) is a no-op, but passing
        # any explicit attn_mask disqualifies SDPA's flash kernel — drop it so the
        # unmasked fast path is taken. (Local change vs the vendored original.)
        if bool(allow.all()):
            self.attn_mask = None
        else:
    ...
    def forward(self, x_btSd: torch.Tensor) -> torch.Tensor:
        ...
        # (1,1,S,S) broadcasts inside SDPA; None = unmasked flash fast path.
        y = self.attn(x, attn_mask=self.attn_mask, is_causal=False)
